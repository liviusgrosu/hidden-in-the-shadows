//Author: Savan Patel and Mr.Lane and Ryan Randive(powerup subprograms)
//File Name: CharacterControl
//Project Name: Final Destiny
//Creation Date: Wed Jan 18 2014
...
//as well as use certain powerups while also determining if they are hit by one
using UnityEngine;
...
public class CharacterControl : MonoBehaviour {
	...
	//Pre: none
	//Post: increase speed
	//Description: makes the player speed greater
	public void PlayerBoost()
	{
		//values of the boosted speed
		float boostSpeed = 5f;
		float jumpBoost = 7f;
		
		//makes the movement speeds assigned to the boost speeds
		playerMovementSpeed = boostSpeed;
		jumpSpeed = jumpBoost;
		StartCoroutine(TimeForPowerup());
		
		//powerup is not equipped
		isBoostEquipped = false;
	}
	
	//Pre: none
	//Post: none
	//Description: makes the powerup last 10 seconds
	...
	//Pre: if player is hit by mine
	//Post: none
	//Description: if player is hit by mine stun them
	...
	//Pre: none
	//Post: none
	//Description: makes player immobile for a few seconds
	IEnumerator TimeOfStun()
	{
		...
		//after 3 seconds player can move again
		yield return new WaitForSeconds(3f);
		playerMovementSpeed = 2f;
		jumpSpeed = 5f;
	}
	
	//displays if the user has a powerup equipped
	void OnGUI () 
	{
		GUI.Box (new Rect (0,0,100,25), "P" + playerNumber);
		if(playerMovementSpeed > 2f)
		{
			GUI.Box (new Rect (0,25,100,25), "Boost Equipped");
		}
	}
